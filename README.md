...
## SQL 
### script to calculate conversion rate

```
SELECT 
    COUNT(DISTINCT(session_id)) AS visits, -- Total unique sessions (visits)
    COUNT(CASE WHEN conversion > 0 THEN 1 END) AS conversions -- Sessions with at least 1 'buy'
FROM 
    (
        SELECT 
            session_id,
            SUM(CASE WHEN EVENT = 'buy' THEN 1 ELSE 0 END) AS conversion -- Total 'buy' events per session
        FROM collector_log
        GROUP BY session_id
    ) c;
...
- In our collector_log table that tracks user interactions, what are the  different types of events recorded, and how many times has each event occurred?"
